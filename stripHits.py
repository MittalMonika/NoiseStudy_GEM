...
for chain_index,evt in enumerate(chain):
    ...
    for RecHit_index in range(0,n_gemrec):
        ...
        #added condition by monika  rechits with cluster size 2 and 3
        if((RecHit_Dict[RecHitEtaPartitionID]['cluster_size'][-1] == 2) or (RecHit_Dict[RecHitEtaPartitionID]['cluster_size'][-1] == 3)) :
...
OutF = ROOT.TFile("/eos/cms/store/group/dpg_gem/comm_gem/P5_Commissioning/2021/Calibration/test/DeadChannel/Output/"+outputname+".root","RECREATE")
#OutF = ROOT.TFile("Output/monika.root","RECREATE")
